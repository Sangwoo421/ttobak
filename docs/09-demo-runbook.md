...
| **Gemini STT 무료 등급은 하루 25회** (`limit: 25, model: gemini-3.5-transcribe`, 429 실측 — 35초 간격으로 4번 다시 시도해도 계속 429) | 25번째 이후 마이크가 전부 빈 문자열 → "잘 못 들었어요" | **리허설 + 본 시연을 합쳐 하루 25턴이 전부다.** 음성 턴은 대본의 3·4·6·7·8 다섯 번 → 리허설 2회 + 본 시연 1회 = 15턴이 한계. 나머지 연습은 개발자 패널 "녹음 클립 입력"(stub_text) 이나 글 입력으로. **결제 등록이 가장 확실한 해결**(유료 등급은 한도가 다르다) |
...
- [ ] 마이크 한 번 말해보기 → 말풍선에 글이 뜬다 (안 뜨면 STT 일일 한도(25회) 소진 → 녹음 클립 입력으로 전환. 이 한 번도 25회에 포함된다)
